...
class HeadMetadataStore:
    """
    A class to store archived metadata.Using jsonl files
    """
    def __init__(self, file_folder: str,workspace:str,cls,log_path: str = "log",read_only: bool = False):
        self.read_only = read_only
        self.workspace=workspace
        print(f"Initializing {cls.__name__} metadata store")
        self.log_path = log_path + "/"+cls.__name__ 
    ...
    def get(self, fork_url: str,range: OsvRange, head: str):
       hash= self.get_hash(fork_url,range,head)
       if not hash in self.range_metadata:
           logging.warning(f"Fork URL {fork_url} {range.vulnerability_id} {head} not found in cached metadata. ")

           if self.read_only:
               logging.warning(f"skip --> read-only mode")
               return -1
           
           range_metadata = self.range_metadata_class(range=range,fork=fork_url,workspace=self.workspace,head=head,log_path=self.log_path).computeMetadata()
           self.range_metadata[hash] = range_metadata
           self._add_line_to_jsonl(self.file_folder, {hash: range_metadata}) 
       #else:
           #logging.warning(f"Fork URL {fork_url} found in cached metadata. Using cached value.{self.range_metadata[hash]}")
        
       return self.range_metadata[hash]
    
    
    def get_hash(self,fork_url: str,range: OsvRange, head: str):
      head=head.replace("swh:1:rev:","")
      return fork_url+"_"+range.vulnerability_id+"_"+head+"_"+str(range.my_hash())
