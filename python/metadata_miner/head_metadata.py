...
class HeadMetadata(RangeMetadata):
  
    def __init__(self,workspace:str, range: OsvRange, fork: str, head:str,log_dir: str = "log"):
        self.head = head
        super().__init__(workspace=workspace, range=range,fork=fork,log_dir=log_dir,)
    ...
    def _setup_logger(self):
            ...
            print("Setting up logger head")
            os.makedirs(self.log_dir, exist_ok=True)
            id=f"{self.repo_owner}_{self.repo_name}__{self.range_repo_name}_{self.range_repo_name}_{str(self.range.vulnerability_id)}_{self.head}_{str(self.range.my_hash())}"
